...
function getRTCPeer(rpcPeer, hsyncClient) {
  if (!peers[rpcPeer.hostName]) {
    peers[rpcPeer.hostName] = createPeerConnection(rpcPeer, hsyncClient);
  }
  return peers[rpcPeer.hostName];
}


function createPeerConnection(rpcPeer, hsyncClient) {
  let peer = { hsyncClient };
  peer.events = new events.EventEmitter();
